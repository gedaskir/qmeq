...
C       =============================================
C
C       2015-10-06 Update: Added conversion of loop variable K from INT to DBLE. Lines 45, 46, 51, 52
C
     ...
     &       -.83333333333333333D-01,.4432598039215686D0/
        ...
        ELSE
           ...
           PSI=TH+0.5D0*Y/Z2
           DO 10 K=1,8
              PSR=PSR+A(K)*Z2**(-DBLE(K))*DCOS(2.0D0*K*TH)
10            PSI=PSI-A(K)*Z2**(-DBLE(K))*DSIN(2.0D0*K*TH)
           IF (X.LT.8.0D0) THEN
              RR=0.0D0
              RI=0.0D0
              DO 20 K=1,N
                 RR=RR+(X0-K)/((X0-DBLE(K))**2.0D0+Y*Y)
20               RI=RI+Y/((X0-DBLE(K))**2.0D0+Y*Y)
              PSR=PSR-RR
              PSI=PSI+RI
           ENDIF
           IF (X1.LT.0.0D0) THEN
