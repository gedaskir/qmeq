...
def test_construct_ham_coulomb():
    ...
    assert construct_ham_coulomb(p.coulomb, statelst, si).tolist() == np.diag([30., 80., 30., 30., 80., 30.]).tolist()
    statelst = [15] # si.chargelst[4]
    assert construct_ham_coulomb(p.coulomb, statelst, si).tolist() == [[280.]]

def test_construct_ham_hopping():
    si = StateIndexing(4, indexing='Lin')
    p = Parameters_double_dot_spinful()
    statelst = [1, 2, 4, 8] # si.chargelst[1]
    assert construct_ham_hopping(p.hsingle, statelst, si).tolist() == [[2, 20, 0, 0], [20, 1, 0, 0], [0, 0, 2, 20], [0, 0, 20, 1]]
    statelst = [3, 5, 6, 9, 10, 12] # si.chargelst[2]
    assert construct_ham_hopping(p.hsingle, statelst, si).tolist() == [[3, 0, 0, 0, 0, 0], [0, 4, 20, 20, 0, 0], [0, 20, 3, 0, 20, 0], [0, 20, 0, 3, 20, 0], [0, 0, 20, 20, 2, 0], [0, 0, 0, 0, 0, 3]]

def test_construct_manybody_eigenstates():
    si = StateIndexing(4, indexing='Lin')
    p = Parameters_double_dot_spinful()
    statelst = [1, 2, 4, 8] # si.chargelst[1]
    ham_vals, ham_vecs = construct_manybody_eigenstates(p.hsingle, p.coulomb, statelst, si)
...
def test_construct_manybody_eigenstates_ssq():
    ...
    p = Parameters_double_dot_spinful()
    #
    eigvalp, eigvecssq = construct_manybody_eigenstates_ssq(2, 0, 0, p.hsingle, p.coulomb, si)
    assert norm(eigvalp - data[(0,0)]) < EPS
    #
    eigvalp, eigvecssq = construct_manybody_eigenstates_ssq(2, -2, 2, p.hsingle, p.coulomb, si)
    assert norm(eigvalp - data[(-2,2)]) < EPS
    eigvalp, eigvecssq = construct_manybody_eigenstates_ssq(2,  0, 2, p.hsingle, p.coulomb, si)
    assert norm(eigvalp - data[( 0,2)]) < EPS
    eigvalp, eigvecssq = construct_manybody_eigenstates_ssq(2, +2, 2, p.hsingle, p.coulomb, si)
    assert norm(eigvalp - data[(+2,2)]) < EPS
    #
    valslst, vecslst = construct_manybody_eigenstates_ssq_all(2, p.hsingle, p.coulomb, si)
...
def test_make_hsingle_mtr_and_dict():
    ...
    assert make_hsingle_dict(hsingle_list) == hsingle_dict
    assert make_hsingle_dict(np.array(hsingle_mtr)) == hsingle_dict
    assert make_hsingle_dict(hsingle_dict) == hsingle_dict
...
def test_make_coulomb_dict():
    ...
    assert make_coulomb_dict(coulomb_list) == coulomb_dict
    assert make_coulomb_dict(coulomb_arr) == coulomb_dict
    assert make_coulomb_dict(coulomb_dict) == coulomb_dict
...
def test_QuantumDot():
    ...
    for indexing in ['Lin', 'charge', 'sz', 'ssq']:
        ...
        if indexing == 'Lin':
            ...
            assert qd.hamlst[0][0].tolist() == [[0.0]]
            assert qd.hamlst[1][0].tolist() == [[2.0, 20.0], [20.0, 1.0]]
            assert qd.hamlst[1][1] == None
            assert qd.hamlst[2][0] == None
            assert qd.hamlst[2][1].tolist() == [[84.0, 20.0, 20.0, 0.0], [20.0, 33.0, 0.0, 20.0], [20.0, 0.0, 33.0, 20.0], [0.0, 20.0, 20.0, 82.0]]
            assert qd.hamlst[2][2] == None
            assert qd.hamlst[3][0].tolist() == [[145.0, 20.0], [20.0, 144.0]]
