...
cdef class ApproachRTD(Approach):
    ...
    cdef void prepare_arrays(self):
        ...
        self.Wdd = np.zeros((nleads, kern_size, kern_size), dtype=self.dtype, order='F')
        ...
        if self.funcp.off_diag_corrections:
            ...
            self.Lnn = np.zeros((kern_size2, kern_size2), dtype=self.dtype, order='F')
    ...
    cpdef void generate_kern(self):
        cdef long_t bcharge, b, bp, bcount, i
    ...
    cdef void add_off_diag_corrections(self, KernelHandlerRTD kh):
        cdef double_t[:,:,:] Wcorr = np.zeros(self.Wdd.shape, dtype=doublenp)
        cdef long_t l
        for l in range(kh.nleads):
            Wcorr.base[l,:,:] = np.matmul(np.matmul(kh.ReWdn.base[l,:,:], kh.Lnn), kh.ImWnd)
            Wcorr.base[l,:,:] = Wcorr.base[l,:,:] + np.matmul(np.matmul(kh.ImWdn.base[l,:,:], kh.Lnn), kh.ReWnd)
        self.Wdd += Wcorr
        cdef long_t kern_size = self.get_kern_size()
    ...
    cdef void generate_row_1st_energy_kernel(self, long_t b, long_t bcharge, KernelHandlerRTD kh):
    ...
    cdef void generate_row_2nd_energy_kernel(self, long_t b, long_t bcharge, KernelHandlerRTD kh):
        cdef long_t i, a, aa, l, lp, n1, c, cc, acount, ccount, acharge, ccharge, nleads, nsingle
        cdef long_t [:, :] statesdm
        cdef double_t mu, Tr, dE, temp, PI
        cdef double_t [:] E, mulst, tlst
    ...
    cdef void generate_matrix_element_2nd_order(self, long_t a0, long_t charge, KernelHandlerRTD kh):
        cdef long_t nleads, acharge, bcharge, ccharge, dcharge, acount, bcount, ccount, dcount
        cdef long_t indx0, indx1, r0, r1, a1p, a2p, a2m, a3p, a3m, i, j, k
        cdef long_t [:, :] statesdm
                ...
                for i in range(ccount):                    
                    ...
                    for j in range(dcount):
                        ...
                        for k in range(ccount):
                            ...
                            if abs(t2D) > t_cutoff:
                                ...
                                kh.add_element_2nd_order(r0, tempD.real, indx0, indx1, a3p, ccharge, a0, bcharge)                                
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(1, 1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a3p, ccharge, a0, bcharge)                                
                        #p2 = -1
                        for k in range(ccount):
                            ...
                            if abs(t2D) > t_cutoff:
                                tempD = t2D * integralD(1, 1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r0, tempD.real, indx0, indx1, a2p, dcharge, a3m, ccharge)
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(1, 1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a2p, dcharge, a3m, ccharge)
                    #p1 = -1
                    for j in range(acount):
                        ...
                        for k in range(bcount):
                            ...
                            if abs(t2D) > t_cutoff:
                                tempD = t2D * integralD(-1, 1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r0, tempD.real, indx0, indx1, a3p, bcharge, a2m, acharge)
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(-1, 1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a3p, bcharge, a2m, acharge)
                        #p2 = -1
                        for k in range(bcount):
                            ...
                            if abs(t2D) > t_cutoff:
                                tempD = t2D * integralD(-1, 1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r0, tempD.real, indx0, indx1, a1p, ccharge, a3m, bcharge)
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(-1, 1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a1p, ccharge, a3m, bcharge)
                    #eta1 = -1
                    for j in range(bcount):
                        ...
                        for k in range(acount):
                            ...
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(1, -1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a3p, acharge, a0, bcharge)
                        #p2 = -1
                        for k in range(acount):
                            ...
                            if abs(t2D) > t_cutoff:
                                tempD = t2D * integralD(1, -1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r0, tempD.real, indx0, indx1, a2p, bcharge, a3m, acharge)
                        for k in range(ccount):
                            a3m = statesdm[ccharge, k]
                            ...
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(1, -1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a2p, bcharge, a3m, ccharge)
                    #p1 = -1
                    for j in range(ccount):
                        ...
                        for k in range(dcount):
                            ...
                            if abs(t2D) > t_cutoff:
                                tempD = t2D * integralD(-1, -1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r0, tempD.real, indx0, indx1, a3p, dcharge, a2m, ccharge)                        
                        for k in range(bcount):
                            a3p = statesdm[bcharge, k]
                            ...
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(-1, -1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a3p, bcharge, a2m, ccharge)
                        #p2 = -1
                        for k in range(bcount):
                            a3m = statesdm[bcharge, k]
                        ...
                        for k in range(dcount):
                            ...
                            if abs(t2X) > t_cutoff:
                                tempX = -t2X * integralX(-1, -1, E1, E2, E3, T1, T2, mu1, mu2, D, b_and_R)
                                kh.add_element_2nd_order(r1, tempX.real, indx0, indx1, a1p, ccharge, a3m, dcharge)


    cdef void generate_col_nondiag_kern_1st_order_dn(self, long_t a1, long_t b1, long_t charge, KernelHandlerRTD kh):
    ...
    cdef void generate_col_nondiag_kern_1st_order_nd(self, long_t a1, long_t charge, KernelHandlerRTD kh):
    ...
    cdef void generate_row_inverse_Liouvillian(self, long_t a1, long_t b1, long_t charge, KernelHandlerRTD kh):
