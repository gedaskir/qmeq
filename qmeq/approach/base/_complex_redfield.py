...
# Redfield approach
#---------------------------------------------------------------------------------------------------------
def generate_kern_redfieldc(sys):
        ...
        for b, bp in itertools.product(si.statesdm[charge], si.statesdm[charge]):
            ...
            if bbp != -1 and bbp_bool:
                ...
                for a, ap in itertools.product(si.statesdm[charge-1], si.statesdm[charge-1]):
                    ...
                    if aap != -1:
                        bpap = si.get_ind_dm1(bp, ap, charge-1)
                ...
                for c, cp in itertools.product(si.statesdm[charge+1], si.statesdm[charge+1]):
                    ...
                    if ccp != -1:
                        cpbp = si.get_ind_dm1(cp, bp, charge)
...
def generate_phi1_redfieldc(sys):
    (phi0, E, Tba, phi1fct, phi1fct_energy, si) = (sys.phi0, sys.qd.Ea, sys.leads.Tba, sys.phi1fct, sys.phi1fct_energy, sys.si)
